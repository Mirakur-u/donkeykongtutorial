...
@onready var hammer_collision: Area2D = $HammerCollision
...
func _physics_process(delta: float) -> void:
	...
	if velocity.x != 0:
		var run_animation_name = "run" if !has_hammer else "run_hammer"
		animated_sprite_2d.play(run_animation_name)
	else:
...
func _on_hammer_collision_body_entered(body: Node2D) -> void:
